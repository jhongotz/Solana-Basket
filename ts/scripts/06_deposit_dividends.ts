...
// Admin deposits dividends into the basket (base token).
(async () => {
  ...
  const amount = new BN(100_000); // deposit 0.1 units
  await program.methods.depositDividends(amount)
    .accounts({
    ...
    .signers([payer])
    .rpc();
  console.log("Deposited dividends");
})();